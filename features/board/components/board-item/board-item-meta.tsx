...
const BoardItemMeta = ({ board }: Props) => {
  const { boardDisabled, rewardText, isCompleted, isTodayDone } =
      ...
      <View className="min-w-0 flex-1">
        ...
        <View className="mt-[2.5px] flex-row items-center gap-[4px]">
          ...
          <AppText
            numberOfLines={1}
            variant="caption2"
            className="min-w-0 flex-1 text-[10.5px] text-gray-400"
          >
            {rewardText}
          </AppText>
        </View>
        ...
        <View className="mt-[6px] flex-row gap-[5px]">
          <View className="rounded-full bg-gray-100 px-[7px] py-[2px]">
            <AppText variant="caption2" className="text-[9.5px] text-gray-400">
              달성 {board.currentCount}/{board.targetCount}
            </AppText>
          </View>
          <View
            className={[
              "rounded-full px-[7px] py-[2px]",
              board.currentStreak >= 10 ? "bg-primary-100" : "bg-gray-100",
            ].join(" ")}
          >
            <AppText
              variant="caption2"
              weight={board.currentStreak >= 10 ? "semibold" : "regular"}
              className={[
                "text-[9.5px]",
                board.currentStreak >= 10
                  ? "text-primary-700"
                  : "text-gray-400",
              ].join(" ")}
            >
