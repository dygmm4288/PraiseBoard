...
import type * as WhaleMessagePolicy from "./whale-message.policy";

const require = createRequire(import.meta.url);
const { resolveWhaleMessage } = require(
  "./whale-message.policy.ts",
) as typeof WhaleMessagePolicy;
...
test("목표가 다른 복수 활성 보드는 각자의 목표를 모두 충족해야 한다", () => {
  ...
  assert.notEqual(resolveTrigger(boards), "daily_limit_reached");
  assert.equal(
    resolveTrigger([
      boards[0],
