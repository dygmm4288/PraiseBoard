...
import { PushPermissionStatus } from "../model/notification.interface";
...
const getExpoPushToken = async () => {
  ...
  if (!projectId) {
    reportError(new Error("EAS projectId is required for push tokens."), {
      scope: "notification.getExpoPushToken.missingProjectId",
      severity: "warning",
    });
    return null;
  }
  ...
  try {
    ...
    reportError(error, {
      scope: "notification.getExpoPushToken",
      severity: "warning",
    });
    return null;
  }
